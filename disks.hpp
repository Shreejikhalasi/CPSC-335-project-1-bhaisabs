...
sorted_disks sort_alternate(const disk_state& before) {
  ...
  for (size_t i = 0; i < after.light_count(); i++) {
    if (i%2) {
      for (size_t y = 1; y < after.total_count() - 1; y = y + 2){
        disk_color swapMe = after.get(y);
        disk_color withMe = after.get(y+1);
        if (swapMe == DISK_LIGHT && withMe == DISK_DARK) {
          after.swap(y);
          counter++;
        }
      }
    }else {
      for (size_t x = 0; x < after.total_count(); x = x + 2) {
        disk_color swapMe = after.get(x);
        disk_color withMe = after.get(x+1);
        if (swapMe == DISK_LIGHT && withMe == DISK_DARK) {
          after.swap(x);
          counter++;
        }
...
sorted_disks sort_lawnmower(const disk_state& before) {
  ...
  for (size_t i = 0; i < after.light_count(); i++){
    if (i%2) {
      size_t index = after.total_count()-1;
      for (size_t y = 0; y < (after.light_count() - 1); y++) {
        index = index - 2;
        disk_color swapMe = after.get(index);
        disk_color withMe = after.get(index+1);
        if (swapMe == DISK_LIGHT && withMe == DISK_DARK) {
          after.swap(index);
          counter++;
        }
